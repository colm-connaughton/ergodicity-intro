...
Note that unlike for the deterministic case where lots of different dynamics, $T$, are studies, for stochastic processes, $T$ is always the shift map (although this map obviously depends in a complicated way on the generator of the process.)
\item
...
Since this observable depends only on a single corrdinate, in this case $(\omega)_0 = x_0$, the integral on the RHS only depends on the one-dimensional marginal of the path space induced measure $\mu$. 
...
Some nonstationary systems revisit any set infinitely often but have infinite expected return time because phase space is infinite - e.g. low dimensional random walk. Meaningful averages can be defined in such cases but infinite ergodic theory has less to do with equality of time and ensemble averages because Birkhoff's Theorem is usually trivial.
