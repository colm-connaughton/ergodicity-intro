...
This is not interpretable as a difference of time and ensemble averages since the time average of $X_n$ is either zero or infinite (depending on the sign of $\lambda$) and the ensemble average is $n$ dependent and also becomes either zero or infinite as $n \to \infty$ (depending on the sign of $\mu$.)
However, there is an underlying Birkhoff style ergodicity statement encoded by Eq.~(\ref{eq-limlogGRW}).
...
Such processes abound in practical time series analysis and signal processing. 
